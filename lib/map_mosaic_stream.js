...
function mapMosaicStream (opts) {
  ...
  var tileStreams = tileViewports.map(function (rows) {
    ...
    return function (cb) {
      ...
      return cb(null, mapImageStream(opts.style, viewport, {
        token: opts.token,
        pixelRatio: dpr
      }))
    }
...
function progress (onProgress) {
  ...
  function onChunk (chunk, enc, next) {
    ...
    if (Date.now() - lastTick < 500 || document.hidden) return next()
    lastTick = Date.now()
...
function getMaxTileSize (pixelRatio) {
  ...
  return Math.floor(
    gl.getParameter(gl.MAX_RENDERBUFFER_SIZE) / pixelRatio
  )
}
